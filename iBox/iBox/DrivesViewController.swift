...
class DrivesViewController: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    private func canCreateNewDrive() -> Bool {
        ...
        if self.configuration!.ataInterfaces?.count > 0 {
            ...
            do {
                let fetchResult = try Store.sharedInstance.managedObjectContext.executeFetchRequest(fetchRequest)
    ...
    override func tableView(tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        headerView.irqStepper.addTarget(self, action: #selector(DrivesViewController.irqStepperValueDidChange(_:)), forControlEvents: UIControlEvents.ValueChanged)
