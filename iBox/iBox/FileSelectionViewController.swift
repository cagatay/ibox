...
class FileSelectionViewController: UITableViewController {
    ...
    @IBAction func refresh(sender: AnyObject) {
        ...
        do {
            let URLs = try NSFileManager.defaultManager().contentsOfDirectoryAtURL(documentsURL, includingPropertiesForKeys: nil, options: []) as [NSURL]
            ...
            BXImage.createImageWithURL(fileURL, sizeInMB: UInt(size), completion: { (success: Bool) -> Void in
                ...
                NSOperationQueue.mainQueue().addOperationWithBlock({ () -> Void in
                    ...
                    // add file name to table with animation if doesn't already exist...
                        ...
                        self.files = (self.files as NSArray).sortedArrayUsingSelector(#selector(NSNumber.compare(_:))) as! [String]
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        // configure cell
        cell.textLabel!.text = (file as NSString).lastPathComponent as NSString as String
