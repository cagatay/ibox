...
class EmulatorViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NSThread.detachNewThreadSelector(#selector(EmulatorViewController.startEmulator), toTarget: self, withObject: nil)
        
        NSThread.detachNewThreadSelector(#selector(EmulatorViewController.startRendering), toTarget: self, withObject: nil)
    }
    ...
    func startRendering() {
        ...
        let timer = NSTimer(timeInterval: 0.01, target: self, selector: #selector(EmulatorViewController.redrawRenderer), userInfo: nil, repeats: true)
    ...
    func exportConfigurationToTemporaryFile(configuration: Configuration) -> String {
        ...
        if configuration.ataInterfaces!.count != 0 {
        ...
        let path = NSURL(fileURLWithPath: NSTemporaryDirectory()).URLByAppendingPathComponent("os.ini", isDirectory: false)?.path
